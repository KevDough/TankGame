...
class Cannon(py.sprite.Sprite):
    ...
    def __init__(self, x_cor, y_cor, dt, img, walls):
    ...
    def update(self):
        ...
        self.wallDetection('x')
        self.hit_rect.centery = self.pos.y
        self.wallDetection('y')
    ...
    def wallDetection(self, dir):  #sets cannon's x velocity to zero if it hits the left or right side of a wall, same idea for y velocity
        if dir == 'x':
            hits = py.sprite.spritecollide(self, self.walls, False, collideHitRect)
            if hits:
                if self.vel.x > 0:
...
class Player(py.sprite.Sprite):
    ...
    def __copy__(self):
        return Player(self.pos.x, self.pos.y, self.dt, self.img, self.walls)
    ...
    def wallDetection(self, dir):  #sets tank's x velocity to zero if it hits the left or right side of a wall, same idea for y velocity
        '''Doesn't let a sprite go past a wall'''
        if dir == 'x':
            hits = py.sprite.spritecollide(self, self.walls, False, collideHitRect)  #collideHitRect is a callback func which is passing a rectangle that doesn't change from player rotation
            if hits:
                if self.vel.x > 0: #if moving to the right
                    self.pos.x = hits[0].rect.left - self.hit_rect.width/2   #set tanks x position on the left side of the wall - its width/2
                if self.vel.x < 0:
                    self.pos.x = hits[0].rect.right + self.hit_rect.width/2
                self.vel.x = 0                    #stop tank from moving in x direction, y direction it can still move
                self.hit_rect.centerx = self.pos.x
                hits.clear()
        if dir == 'y':
            hits = py.sprite.spritecollide(self, self.walls, False, collideHitRect)
...
class Enemy(py.sprite.Sprite):
    ...
    def __init__(self, x_cor, y_cor, img, dt, player_pos, walls, type):
        py.sprite.Sprite.__init__(self)
        self.width, self.height = 64,64
        self.img = img
        self.type = type
